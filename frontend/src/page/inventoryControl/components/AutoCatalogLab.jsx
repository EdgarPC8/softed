  ...
  useEffect(() => {
    const fetchSeed = async () => {
      try {
        setLoading(true);
        setError(null);
        const { data } = await getAutoCatalogSeed({
          days: 30,
          limit: 50,        // traemos más y recortamos en UI con 'limit'
          onlyActive: true,
          // section: targetSection, // opcional (si quieres traer catálogo filtrado por sección)
        });
      ...
      <Box sx={{ mt: 1 }}>
        ...
        <Paper variant="outlined" sx={{ p: 0.75 }}>
          <Stack direction="row" spacing={0.5} flexWrap="wrap">
            {catalogRows
              ...
              .map((r) => {
                ...
                return (
                  <Chip
                    key={r.id}
                    label={`${r.title || prod?.name} · ${currency(
                      r.priceOverride ?? prod?.price
                    )}`}
                    variant="outlined"
                    size="small"
                    sx={{
            ...
            {catalogRows.filter((r) => r.section === targetSection).length === 0 && (
              <Typography variant="caption" color="text.secondary">
                (Vacío)
              </Typography>
            )}
