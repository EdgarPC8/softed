...
function ProductForm({ isEditing = false, datos = {}, onClose, reload }) {
  ...
  const submitForm = async (data) => {
    ...
    // ✅ 1) Subfolder primero (IMPORTANTE para multer destination)
    // Debe respetar lo que escribes: "", "EdDeli", "EdDeli/products", etc.
    const subfolder = String(imageSubfolder || "")
      .trim()
      .replace(/\\/g, "/")      // windows -> linux
    ...
    // ✅ 3) opcional: nombre base del archivo
    fd.append("customFileName", data.name?.trim() || "producto");
  
    // ✅ 4) Archivo AL FINAL (IMPORTANTE)
