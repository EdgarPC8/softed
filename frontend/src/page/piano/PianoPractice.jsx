...
const SynthesiaRoll = () => {
  ...
  // Overlay (teclas presionadas por PC/MIDI)
  ...
  // anti-repeat
  const activePcKeysRef = useRef(new Set());
  ...
  // Mapeos PC
  const PC_WHITE = useMemo(() => ({
    C4: 'a', D4: 's', E4: 'd', F4: 'f', G4: 'g', A4: 'h', B4: 'j',
    C5: 'k', D5: 'l', E5: ';'
  }), []);
  const PC_BLACK = useMemo(() => ({
    'C#4': 'w', 'D#4': 'e', 'F#4': 't', 'G#4': 'y', 'A#4': 'u',
    'C#5': 'o', 'D#5': 'p'
  }), []);
  const pcKeyForNote = (note) => PC_WHITE[note] || PC_BLACK[note] || null;
  ...
  const notesToPlay = useMemo(() => {
    return rawNotes.map((n, idx) => {
      ...
      return { ...n, note: nn, key: pcKeyForNote(nn), _idx: idx };
    });
  }, [rawNotes]);

  // Rango visible 4 octavas: C2 a E5
  ...
  // Overlay (tecla -> nota)
  const staticOverlayMap = useMemo(() => ({
    a: 'C4', s: 'D4', d: 'E4', f: 'F4', g: 'G4', h: 'A4', j: 'B4',
    k: 'C5', l: 'D5', ';': 'E5',
    w: 'C#4', e: 'D#4', t: 'F#4', y: 'G#4', u: 'A#4', o: 'C#5', p: 'D#5',
  }), []);
  const dynamicOverlayMap = useMemo(() => {
    const map = {};
    for (const n of notesToPlay) if (n.key) map[n.key] = n.note;
    return map;
  }, [notesToPlay]);
...
useEffect(() => {
  ...
  const drawPiano = () => {
    ...
    blackKeyNotes.forEach((note) => {
      ...
      if (idx !== -1 && base !== 'E' && base !== 'B') {
        ...
        let fillColor = 'black';
        if (highlightedKeys[note]) fillColor = highlightedKeys[note];
        else if (overlayPressedNotesRef.current.has(note)) fillColor = overlayKeyColor;
    ...
    if (isPlaying && autoSim && (groups[currentGroupIdx] || []).length && currentGroupAllAtKey) {
      if (lastAutoAdvancedGroupRef.current !== currentGroupIdx) {
        for (const n of groups[currentGroupIdx]) {
          // highlight por mano (esto también dispara repintado del piano via state)
          setHighlightedKeys(prev => ({ ...prev, [n.note]: handColors[n.hand] }));
          setTimeout(() => {
  ...
  const handleKeyDown = (e) => {
    const key = (e.key || e).toLowerCase();
    if (key === 'arrowleft' || key === 'arrowright') return;
    if (e.repeat) return;
    if (activePcKeysRef.current.has(key)) return;
    activePcKeysRef.current.add(key);

    const overlayNote = (staticOverlayMap[key] ?? dynamicOverlayMap[key]);
    if (overlayNote && visibleRange.includes(overlayNote)) {
      overlayPressedNotesRef.current.add(overlayNote);
      bumpOverlay();
    }
    ...
    const target = currentGroup.find(n => n.key === key);
    if (!target) return;

    const ok = canAcceptHitNow() || isNoteAtKeyNow(target);
    if (!ok) return;
  ...
  const handleKeyUp = (e) => {
    const key = (e.key || e).toLowerCase();
    if (activePcKeysRef.current.has(key)) activePcKeysRef.current.delete(key);
    const overlayNote = (staticOverlayMap[key] ?? dynamicOverlayMap[key]);
    if (overlayNote && overlayPressedNotesRef.current.has(overlayNote)) {
      overlayPressedNotesRef.current.delete(overlayNote);
      bumpOverlay();
    }
  };
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isPlaying, currentGroupIdx, groups, staticOverlayMap, dynamicOverlayMap, visibleRange]);
  ...
  const startPlayback = async () => {
    ...
    activePcKeysRef.current = new Set();
      ...
      <Typography sx={{ mt: 1 }} variant="body2" color={
        ...
        {phase === 'playing'
          ? (autoSim
            ...
            : (noteReady ? '👉 Toca TODAS las notas del grupo casi a la vez.' : '⏳ Espera a que el grupo toque el teclado…'))
          : phase === 'paused'
