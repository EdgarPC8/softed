...
/** Modo Práctica: eliges canción de la BD y practicas con teclado/MIDI */
export default function ModePractice({ onBack }) {
  const { toast } = useAuth();
  ...
  const handleLoadSong = useCallback(
    async (id) => {
      try {
        ...
        toast({ info: { description: `Practicando: ${payload.title}` } });
      } catch (err) {
    ...
    <Box sx={{ p: 2, width: '100%', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6">Modo práctica</Typography>
          {selectedTitle && (
              ...
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="center">Practicar</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
          ...
          {selectedNotes.length === 0 ? (
            ...
            >
              <Typography variant="body2">
                Elige una canción de la tabla para practicarla aquí con el teclado o con tu dispositivo MIDI.
              </Typography>
            </Box>
          ) : (
            <SynthesiaRoll mode="practice" notes={selectedNotes} bpm={selectedBpm} onBack={onBack} />
          )}
        </Box>
      </Box>
    </Box>
